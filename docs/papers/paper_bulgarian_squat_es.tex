...
\caption{Matriz de confusión normalizada del modelo BiGRU+Attention en el conjunto de test (2,475 muestras). El modelo muestra mejor rendimiento en E3 (profundidad, 88\% recall) y E0 (correcta, 77\% recall), con confusiones principales entre E1-E2 debido a superposición de patrones biomecánicos donde inclinación del tronco puede coexistir con desviación de rodilla.}
...
\caption{Métricas de clasificación por clase en el conjunto de test. E3 (profundidad) muestra mejor rendimiento (F1=89\%, P=90\%, R=88\%), seguido de E0 (correcta, F1=78\%) y E1 (tronco, F1=70\%). E2 (valgo) es la clase más desafiante (F1=41\%) debido a sutileza del movimiento lateral y limitaciones de estimación 2D.}
...
\begin{itemize}
    \item \textbf{E3 (Profundidad insuficiente):} Mejor clase con F1=89\%, demostrando que el modelo captura efectivamente patrones de ROM limitado en ángulos de rodilla y cadera durante la fase de máxima flexión.
    
    \item \textbf{E0 y E1 (Correcta y Tronco):} Rendimiento sólido a moderado (F1=78\% y 70\%), indicando buena capacidad de distinguir entre ejecución correcta e inclinaciones sutiles del tronco, aunque persiste confusión del 12\% entre ambas clases.
    
    \item \textbf{E2 (Valgo de rodilla):} Clase desafiante (F1=41\%) debido a: (1) movimiento lateral sutil difícil de capturar con estimación 2D, (2) confusión con E1 (18\%) por compensaciones biomecánicas donde valgo coexiste con inclinación del tronco, (3) menor saliencia visual en landmarks de rodilla comparado con ángulos de tronco o profundidad.
\end{itemize}
...
\caption{Visualización de pesos de atención promedio a través de pasos temporales para cada clase de error. El mecanismo de atención aprende a enfocarse en fases críticas del movimiento: E0 (correcta) distribuye atención uniformemente, E1 (tronco) se concentra en la fase descendente donde la inclinación es más pronunciada, E2 (valgo) atiende momentos de desviación lateral de rodilla, y E3 (profundidad) se enfoca en el punto de máxima flexión.}
...
El modelo BiGRU+Attention logra \textbf{65.74\% de precisión} y \textbf{51.98\% Macro-F1}, demostrando capacidad sólida para clasificación multi-clase en presencia de patrones de movimiento complejos y superpuestos. El rendimiento destacado en E3 (F1=89\%) y E0 (F1=78\%) indica que el sistema detecta confiablemente profundidad insuficiente y ejecución correcta, proporcionando valor práctico para retroalimentación correctiva en rehabilitación.

\textbf{Impacto crítico del data augmentation:} El hallazgo más significativo de este estudio es la efectividad del balanceo de clases mediante augmentation para habilitar aprendizaje efectivo en clasificación multi-clase. Al crear 16,501 muestras balanceadas (25\% por clase) a partir de datos originales desbalanceados, el modelo logró:

\begin{enumerate}
    \item Aprendizaje efectivo de las 4 clases sin colapso hacia la clase mayoritaria
    \item Convergencia estable en época 9/50 con early stopping apropiado
    \item Representación equitativa en train/val/test mediante GroupShuffleSplit por video\_id
\end{enumerate}

El dataset balanceado combinado con división estratificada permitió que el modelo aprenda características distintivas de todas las clases, logrando F1-scores diferenciados: E3=89\%, E0=78\%, E1=70\%, E2=41\%.
...
\begin{itemize}
    \item \textbf{Simoes et al.} \cite{simoes2024accuracy} reportan 99.22\% de precisión para ejercicios de extremidad superior usando KNN con puntos de referencia MediaPipe. Su tarea es más simple (2 clases, movimientos restringidos) vs. nuestro enfoque multiclase con dinámicas complejas de tren inferior. Nuestro 65.74\% accuracy refleja la mayor complejidad de distinguir entre 4 clases de patrones de movimiento superpuestos.
    
    \item \textbf{Mennella et al.} \cite{mennella2023deep} logran 89\% de precisión para clasificación de ROM (rango de movimiento) y 98\% para detección de patrones compensatorios usando CNNs. Su enfoque procesa fotogramas individuales vs. nuestro modelado explícito de secuencias temporales, que captura dinámicas de movimiento completas pero enfrenta mayor complejidad en clasificación multi-clase.
    
    \item \textbf{Chander et al.} \cite{chander2025rgb} reportan mejora promedio de 12.9\% sobre baselines LSTM usando transformers espacio-temporales. Nuestro estudio de ablación muestra mejora similar (+13.44 puntos accuracy) de BiGRU+Attention sobre BiLSTM baseline, validando el valor de mecanismos de atención para modelado temporal de ejercicios.
\end{itemize}
